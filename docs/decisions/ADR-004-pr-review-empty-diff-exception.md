...
3. **제외된 것 전부가 auto-PASS 자격이 있다** (`[ ! -s .../unsafe-filtered.txt ]`) —
   ...
   - **이미지/문서 자산 확장자** (`png|jpg|jpeg|gif|pdf`, `ascii_downcase` 후 매칭 —
     ...
     **차단된다** — 패널이 읽는 것이 아니다(round-7 리뷰 L5: 바이너리 컨테이너는 patch
     가 없어 패널이 읽을 수 없고, 실제 귀결은 혼합 PR 에서 `filtered`+`unsafe-filtered`
     경고, 단독 PR 에서 전제조건 ③ fail-close 다). `pdf` 는 남겼다 — 그것도 구조화 컨테이너지만, 삭제 조건 아래에서는
...
7. **필터 목록과 예외 범위는 커플링되어 있다 — 영향도는 목록마다 다르다.** `ASSET_RE`
   와 state 패턴(`STATE_RE`/`STATE_GENERIC_RE` — 삭제가 auto-PASS 경로다) 확대는
   auto-PASS 범위를 **직접** 넓힌다(전제조건 ③ 이 걸러주지 않는다). `NOISE_RE` 확대는
   패널이 보는 것을 줄이지만 auto-PASS 자격은 전제조건 ③ 이 그대로 심사한다(텍스트
   noise 의 추가·수정은 여전히 unsafe).
...
- **이미지·문서 자산의 삭제 전용 PR 에 대한 심사가 없다.** — **삭제 전용**만이다: 같은
  자산의 추가·수정은 여전히 `unsafe-filtered` 로 fail-close 한다(round-7 리뷰 L5: 이
  문장이 전에는 "자산 전용 PR" 이라 실제 범위보다 넓게 읽혔다). 패널은 원래 이런 파일을
  읽지 못했으니 *패널* 커버리지가 줄지는 않지만, 삭제 전용의 결과가 "머지 불가" →
  "무심사 머지 가능"으로 바뀌므로 실효 통제는 분명히 약해진다. 그게 이 ADR 이 감수하는 것이다. PNG 에 시크릿이나 악성
...
- **provider 범프 PR 에서는 lockfile 쪽이 절단된다.** `.terraform.lock.hcl` 승격 × 리전
  수만큼의 fan-out 이면 lock 파일 해시 수백 줄이 `head -3000` 예산을 잠식한다.
  `collect-diff.sh` 가 lockfile 을 panel.diff **맨 뒤**로 정렬하므로(test #18) 잘리는
  쪽은 lockfile 해시이고 앞의 `.tf` 변경은 살아남는다 — 즉 리전 간 해시 parity 검증이
  침식되는 것이 실제 비용이다(round-7 리뷰: 이전 서술 "뒤따르는 `.tf` 가 절단된다" 는
  정렬 이후 성립하지 않는다). 절단 사실은 코멘트에 배너로 표시된다. 파일당 줄 캡은
  후속 작업으로 남긴다.
- **경로 매니페스트는 16KiB 에서 잘린다 — "전체 목록" 이 아니다** (round-5 리뷰 L2
  MAJOR). 매니페스트는 codex 셀의 단일 argv 인자에 합쳐지므로 무캡이면 큰 PR 에서
  exec 자체가 실패한다(ARG_MAX). 캡은 그래서 남고, 문제는 절단 **방향** 이었다:
  `sort -u` 된 목록의 뒤쪽 — 사전순으로 `us-west-2` 같은 특정 리전 — 이 큰 PR 에서
  결정론적으로 먼저 사라지고, "N more paths omitted" 만으로는 렌즈가 리전 parity 를
  검증할 수 없었다. 이제 생략분은 디렉터리(앞 4세그먼트)별 카운트로 요약되어 렌즈
  프롬프트 끝에 붙고, 같은 요약이 `manifest-capped.txt` 로 의장 stdin 에도 실린다.
  요약 자체도 4KiB 에서 잘린다(카운트 내림차순 — 가장 많이 잘린 디렉터리가 남는다;
  round-6 리뷰: 디렉터리 수의 상한은 파일 수라 무캡 요약은 ARG_MAX 결함의 재도입이었다).
  렌즈 preamble 의 보장 상한은 16KiB + 4KiB + 고정 문구다; 의장용 파일은 무캡이다.
  수백~2999 파일 PR 에서 렌즈는 "어느 디렉터리에 몇 개가 안 보이는지"(큰 것부터) 는
  알지만 그 파일명은 모른다 — 그 구간의 한계로 기록한다.
