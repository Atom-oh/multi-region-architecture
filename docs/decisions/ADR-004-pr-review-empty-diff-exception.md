...
3. **제외된 것 전부가 git 기준 바이너리이고, 삭제이며, 이미지 자산이다**
   ...
   - **이미지 확장자** (`png|jpg|jpeg|gif|pdf`) — 삭제 조건이 이미 대부분을 덮지만,
...
- 스킵 목록에 `*.tfstate`/`*.tfstate.backup`/`*.tfplan` 를 추가했다. 실수로 커밋되면
  텍스트로 판정되어 패널 프롬프트(4모델×4렌즈)에 state 내용물이 그대로 실린다. 스킵되면
  전제조건 3번(텍스트)에 걸려 fail-close 하므로, 조용히 통과하지 않고 사람이 본다.
