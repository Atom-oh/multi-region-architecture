...
variable "vpc_id" {
  description = "VPC ID where the NLB target groups will be created"
  type        = string
}

variable "public_subnet_ids" {
  description = "List of public subnet IDs for the NLB (multi-AZ)"
  type        = list(string)
}
...
variable "target_groups" {
  description = "Map of target groups to create (key = identifier, value = config)"
  type = map(object({
    name   = string
    weight = number
  }))
