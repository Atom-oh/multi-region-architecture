...
멀티 리전 쇼핑몰 플랫폼은 각 리전에 **Amazon EKS** 클러스터를 배포합니다. Kubernetes 1.36을 사용하며, **Karpenter**를 통해 워크로드별 최적화된 노드를 자동으로 프로비저닝합니다.
...
resource "aws_eks_cluster" "main" {
  ...
  version  = var.cluster_version  # "1.36" — new-cluster default only, see below
...
:::info 왜 version을 ignore_changes에 넣었나
EKS 클러스터는 콘솔/eksctl/auto-upgrade로 out-of-band 업그레이드가 가능하지만 다운그레이드는 불가능합니다. `cluster_version` 변수(현재 기본값 `1.36`)를 terraform이 계속 추적하면, 실제 클러스터가 그 이후 버전으로 올라간 순간부터 되돌릴 수 없는 plan diff가 영구적으로 남게 됩니다. `version`을 `ignore_changes`에 넣어 신규 클러스터 생성 시에만 이 변수가 적용되게 하고, 이후 버전 업그레이드는 terraform 밖에서 수행합니다.
:::
