...
- `terraform/environments/production/ap-northeast-2/` — Korea region (subdirs: `shared/`, `eks-az-a/`, `eks-az-c/`). The `mall-apne2-mgmt` hub cluster is **owned by the `AWS-Demo-Platform` repo** (`infra/eks-mgmt`, applied by its Atlantis) and holds the `production/ap-northeast-2/eks-mgmt/terraform.tfstate` key in this repo's bucket — never apply that state from here. The spokes read the one value they need from it (mgmt's cluster SG, for cross-cluster ArgoCD access) via the `modules/security/mgmt-cluster-trust` module's live `data "aws_eks_cluster"` lookup, so `eks:DescribeCluster` on that cluster is required and **mgmt must exist at plan time**. All five trust inputs — `mgmt_cluster_name`, `default_mgmt_cluster_name` (the rename baseline `mgmt_cluster_name` is compared against), `expected_mgmt_vpc_id`, `expected_mgmt_tags`, and the break-glass `mgmt_cluster_security_group_id_override` — live in `shared/` and are read by both spokes from its state, so the two can never be asked to trust different values (each still needs its own apply). `describable_cluster_names` (the IAM grant letting spokes describe mgmt live) is derived from `mgmt_cluster_name` — a rename is a single `shared/` apply, not a separate edit to keep in sync. Break-glass goes in `shared/terraform.tfvars`, not `-var`, or the next routine `shared/` apply reverts it. Every release of the four monitored trust inputs is named in the plan by `check "mgmt_guards_engaged"` and in state by `output "mgmt_guards_released"` (the acknowledgment flag `break_glass_confirm` is not a trust input and is instead gated by the `terraform_data.break_glass_gate` preconditions in `shared/` and the module, and reported by `check-mgmt-guards.sh`); `bash scripts/check-mgmt-guards.sh` turns that warning into an exit code and also verifies the two spokes converged (including a 5-input `mgmt_trust_fingerprint` 3-way comparison against `shared/`). `ci_runner` (owned by the other repo, `AmazonS3FullAccess`) is to be kept out of this repo's state by an **allowlist bucket policy** in `terraform/global/terraform-state` — **exists in code only until the manual rollout in ADR-003 apply order ⓐ–ⓔ is performed (ⓔ = migrating that layer's own state into the bucket, same session); today the bucket has no policy — update this sentence when ⓔ is done** — per key in three groups: this repo's keys are writable only by `state_custody_appliers` (devbox roles, SSO admin) and additionally readable by `state_custody_readers` (the CI plan role); the eks-mgmt key is writable/readable by **exactly** `external_state_appliers` (the other repo's Atlantis + terraformer, plus SSO admin as break-glass — not the devbox roles); `shared/` is additionally readable by `external_state_readers` (the frozen six-output contract — a whole-object read, so a plaintext-secret grant until `manage_master_user_password` lands; ADR-003 round-18 expiry). Everyone else is denied on the objects and on the bucket's own policy document — including any session `ci_runner` pivots into via its `sts:AssumeRole`/`iam:PassRole` grants. Adding an applier is a policy change; a missing one blocks that layer's next apply (fail-closed — confirm the four lists against the account before applying; the apply itself refuses to run from a principal not on the applier list; ADR-003 rounds 15–18). An identity Deny cannot reach a principal another repo grants. Full contract — plan-time coupling, break-glass path, frozen shared outputs: `docs/decisions/ADR-003-eks-mgmt-ownership-handoff.md`.
