...
def _container_spans(value, key, skip_quoted=False):
    ...
    closing = {"[": "]", "(": ")", "{": "}"}
    spans, cursor = [], 0
    quoted_ranges = list(_json_literal_ranges(value)) if skip_quoted else []
    quoted_index = 0
    while match := opening.search(value, cursor):
        start, index = match.end() - 1, match.end()
        while quoted_index < len(quoted_ranges) and quoted_ranges[quoted_index][1] <= start:
            quoted_index += 1
        if (quoted_index < len(quoted_ranges)
                and quoted_ranges[quoted_index][0] < start < quoted_ranges[quoted_index][1]):
            cursor = quoted_ranges[quoted_index][1]
            continue
...
def _quoted_key_spans(value):
    ...
    for start, end in _json_literal_ranges(value):
        if start < cursor:
            continue
        try:
            key = strict_json(value[start:end])
        except Invalid:
            continue
        ...
        if item:
            ...
            if following < len(value) and value[following] == ":":
                continue  # A quoted key after an empty YAML entry is not its value.
...
def _assignment_spans(value, key, json_closers=None, *, single_strings=()):
...
def scrub(value, preserved=frozenset()):
    ...
    try:
        ...
        pass
    def syntax_literal(match):
        try:
            return canonical(strict_json(match.group()))
        except Invalid:
            return match.group()
    # Validate original container syntax before value redaction can repair it.
    syntax = _normalize_container_keys(re.sub(r'"(?:\\.|[^"\\])*"', syntax_literal, value))
    syntax_key = SENSITIVE_KEY.pattern + r"""(?:\\*["'])?\s*[:=]\s*"""
    original_spans = _container_spans(syntax, syntax_key, skip_quoted=True)
    if original_spans and original_spans[-1][1] == len(syntax):
        return scrub(syntax[:original_spans[-1][0]], preserved) + "[REDACTED]"
    def quoted(match):
        try:
    ...
    patterns = (
        ...
        (key + block, 'block'),
        (key + r"<<-?(?P<heredoc>\w[\w-]*)[ \t]*\r?\n.*?(?:(?m:^[+-]?[ \t]*(?P=heredoc)[ \t]*\r?$)|\Z)", 'heredoc'),
        (rf"(?i:\b(?:header)?name)(?:{quote})?\s*[:=]\s*(?:{quote})?" + identifier
        + rf"(?:{quote})?[\s,]*[+-]?[ \t]*(?:{quote})?(?i:(?:header)?value)(?:{quote})?\s*[:=]\s*"
        + rf"(?P<owned_value>{block}|{string}|[^\s,}}\]]+)", 'named'),
        _quoted_key_spans,
        (key + string, 'scalar'),
        _assignment_spans,
        key + r"""[^\s"',;}\]]+""",
    )
    spans = _container_spans(value, key)
    bodies = []
    for start, end in spans:
        prefix = re.compile(key).match(value, start)
        if prefix.end() + 1 < end - 1:
            bodies.append((prefix.end() + 1, end - 1))
    scan_value = _opaque_scan_view(value, bodies)
    for entry in patterns:
        if entry is _quoted_key_spans:
            owned = _quoted_key_spans(scan_value)
            spans.extend(owned)
            bodies.extend(owned)
            scan_value = _opaque_scan_view(value, bodies)
            continue
