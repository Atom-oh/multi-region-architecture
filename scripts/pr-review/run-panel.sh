...
# 격리 cwd/HOME(co-agent PR 게이트의 `_review_one`/`_sanitized_env`와 동일 패턴, ADR-002 의
# TOCTOU 심링크 가드는 그대로 유지)은 이제 잔여 read 위험의 완화가 아니라 순수
# defense-in-depth(캐시/세션 상태가 실행 간 전이되는 재현성 문제 예방) — 비-ephemeral
# 러너에서 $WORK 가 재사용돼도 매 실행 시작 시 리셋.
