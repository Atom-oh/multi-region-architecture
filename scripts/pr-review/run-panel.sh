...
# 멈춤 방지. 셀이 비면 최대 PANEL_RETRIES 회 재시도(gpt-5.6 계열/bedrock-mantle 등 transient
# 흡수). 매 시도마다 재실행.
...
if command -v kiro-cli >/dev/null 2>&1; then
  if kiro-cli chat --help 2>/dev/null | grep -qF -- "trust no tools: '--trust-tools='"; then
    ...
    for entry in "${KIRO_MODELS[@]}"; do
      ...
      # 준다. 재시도 없이 단발 실패로 바로 skip 하면, gpt-5.6-sol/kiro-cli 류의 흔한 일시
...
for lens_file in "${LENS_FILES[@]}"; do
  ...
  # KIRO_MODELS 의 gpt-5.6-sol 와는 별개 문자열 — Kiro 의 cross-vendor 라우터 카탈로그와
