...
class ReviewFormatTests(unittest.TestCase):
    ...
    def test_closed_example_delimiters_survive_existing_value_guards(self):
        for body in ('password = (old or\n "synthetic-private")',):
    ...
    def test_example_fences_do_not_repair_uncertain_container_syntax(self):
        body = 'password = ["synthetic-private" broken]'
        reply = (0, "Example:\n```text\n" + body +
                 "\n```\nPUBLIC_AFTER\nVERDICT: PASS\n", "")
        calls, text = self.chair([reply, reply])
        self.assertEqual(calls, 2)
